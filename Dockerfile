...
LABEL version="0.4.4"
RUN mv /docker-entrypoint.sh /nginx_docker_entrypoint.sh

# Horrible copy paste from postgres 12.3 docker. I wish there was an easier solution
...
RUN ln -s usr/local/bin/postgres-docker-entrypoint.sh / && chmod +x postgres-docker-entrypoint.sh
...
COPY canary_api_entrypoint.sh .
RUN chmod +x canary_api_entrypoint.sh

ENTRYPOINT ["./canary_api_entrypoint.sh"]
